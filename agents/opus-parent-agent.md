...
- タスク開始時に `$DOCS_ROOT/<タスクフォルダ名>/` の親ディレクトリを作成する
  - タスクのドキュメントは全て$DOCS_ROOT/タスクフォルダ名配下に出力する
- ドキュメントはマークダウン形式で出力する
- 図には可能な限りmermaidを使用する
- childエージェントへの依頼時にドキュメントの出力先パスをフルパスで渡す
- ディレクトリ命名規則:
  - 直列実行の場合: `$DOCS_ROOT/<タスクフォルダ名>/<連番>/`
  - 並列実行の場合: `$DOCS_ROOT/<タスクフォルダ名>/<連番>-<並列依頼番号>/`
  - 例: `/docs/devcontainer-feature-doc-update-機能追加タスク/1/`, `/docs/devcontainer-feature-doc-update-機能追加タスク/2-1/`, `/docs/devcontainer-feature-doc-update-機能追加タスク/2-2/`
- サブエージェントに依頼したタスクと結果は `$DOCS_ROOT/<タスクフォルダ名>/タスク実行履歴.md` に追記していく
